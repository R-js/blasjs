...
export function copyMatricesToWasmMemory(
  forceHiFP: boolean,
  ...rest: unknown[]
): { storage: WebAssembly.Memory; byteLength: number; hiFP: boolean } {
  ...
  const matrices = rest.slice(startIdx) as (
    | ArrayLike<number>
    | Float64Array
    | Float32Array
  )[];
...
export function mapWasmMemoryToMatrices(
  forceHiFP: boolean,
  ...
  if (byteLength > storage.buffer.byteLength) {
    throw new Error(
      `You are asking for more bytes then there is available in storage ${byteLength} > ${storage.buffer.byteLength} `
    );
  }
  byteLength = 0;
  const matrices: (Float64Array | Float32Array)[] = Array.from({
