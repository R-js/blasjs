...
c SUBROUTINE ZHER2K(UPLO,TRANS,N,K,ALPHA,A,LDA,B,LDB,BETA,C,LDC)

      EXTERNAL ZHER2K, COPY, ZEROA, COPYMA,FILL
...
*>    (nxn) = complex(alpha)*(nxk)(kxn)  +     conj(alpha)*(nxk)*(kxn)+real(beta)*(nxn)
*>    (nxn) = alpha*(
*> 
        PRINT *, "==CASE 0=="
*>
*>    C := alpha*A**H*B          + conjg( alpha )*B**H*A + beta*C,
*>    (nxn) = alpha *(nxk)*(kxn) +  alpha*(nxk)*(kxn)+beta*(nxn)
*>
*>    TRANS = 'N' or 'n'    
*>       C := alpha*A*B**H + conjg( alpha )*B*A**H + beta*C.
*>
...
*>           k  when  TRANS = 'N' or 'n',  and is  n  otherwise.
*>           Before entry with  TRANS = 'N' or 'n', 
