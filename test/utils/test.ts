...
describe('blas f_func utility functions', function n() {
    describe('mul_cxc', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { a, b }, expect, desc }, key) => {
    ...
    describe('mul_rxc', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { ra, ia, b }, expect, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('multiplexer', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { i1, i2, i3 }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
                    const c = multiplexer(i1, i2, i3)((a, b, c) => `${a}-${b}-${c}`);
                    expect(c).toEqual(should.c);
                    expect(should.c).toEqual(c);
                });
            });
        });
        describe('test errors', () => {
            const { multiplexerErr: errors } = fixture;
            each(errors)(({ input: { i1, i2 }, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('coerceToArray', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { i }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
                    const ans = coerceToArray(i);
                    expect(ans).toEqual(should.o);
                });
            });
        });
        describe('test errors', () => {
            const { coerceToArrayErr: errors } = fixture;
            each(errors)(({ input: { o }, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('arrayrify', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { fn, data }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('numberPrecision', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { prec, data }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('fortranArrComplex32', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { data, index, re, im }, expect: should, desc }, key) => {
                //
                if (/^case[0-9]{1,}$/.test(`${key}`)) {
                    it(`[${key}]/[${desc}]`, function t() {
                        const f32 = fortranArrComplex32(data)();
                        const typeName = f32.r[Symbol.toStringTag];
                        expect(f32.toArr()).toEqual(should.o);
                        expect(typeName).toEqual(should.type);
                    });
                    return;
                }
                //
        ...
        describe('error tests', () => {
            ...
            each(testData)(({ input: { data, index, re, im }, desc }, key) => {
                if (/^case[0-9]\/s\(\)/.test(`${key}`)) {
                    it(`[${key}]/[${desc}]`, function t() {
                        const f32 = fortranArrComplex32(data)();
    ...
    describe('fortranMatrixComplex32', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { data, dim1, dim2 }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
                    //
                    ...
                    if (fm32.i) {
                        expect(typeNameImagin).toEqual(should.type);
                    }
                    expect(typeNameReal).toEqual(should.type);

                    expect(_5(fm32.toArr())).toEqual(should.m32);
                });
            });
        });
    });
    describe('imaginary', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { data }, expect: should, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
                    //
                    const im = imaginary(data);
                    //console.log(im);
                    const _5: (number) => number = numberPrecision(5) as any;
                    expect(_5(im)).toEqual(should.o);
                });
            });
        });
    });
    ...
    describe('mimicMatrix', () => {
        describe('data tests', () => {
            ...
            each(testData)(({ input: { data, nrCol, nrRow }, expect: should, desc }, key) => {
                if (key === 'case0' || key === 'case1') {
                    it(`[${key}]/[${desc}]`, function t() {
                        ...
                        const _5: (number) => number = numberPrecision(5) as any;
                        expect(_5(imCopy.toArr())).toEqual(_5(should.o));
                    });
                }
                if (key === 'setLower0') {
                    it(`[${key}]/[${desc}]`, function t() {
                        ...
                        expect(_6(imCopy.toArr())).toEqual(_6(should.o));
                    });
                }
                if (key === 'setUpper0') {
                    it(`[${key}]/[${desc}]`, function t() {
                ...
                if (key === 'upperBand0') {
                    it(`[${key}]/[${desc}]`, function t() {
                        ...
                        const _6: (number) => number = numberPrecision(6) as any;
                        expect(_6(imCopy)).toEqual(_6(muxCmplx(should.o.re, should.o.im)));
                    });
                }
                if (key === 'lowerBand0') {
                    it(`[${key}]/[${desc}]`, function t() {
                        ...
                        const _6: (number) => number = numberPrecision(6) as any;
                        expect(_6(imCopy)).toEqual(_6(should.o));
                    });
                }
        ...
        describe('test errors', () => {
            ...
            each(errors)(({ input: { lda, nrCols, rowBase, colBase }, desc }, key) => {
                it(`[${key}]/[${desc}]`, function t() {
    ...
    describe('errMsg', () => {
        describe('data tests', () => {
            it(`default message`, function t() {
                ...
                expect(text).toEqual('Unkown Error code used! [42]');
            });
        });
    });
    /*
