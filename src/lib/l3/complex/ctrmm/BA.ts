...
export function BA(
    ...
    if (upper) {
        for (let j = n; j >= 1; j--) {
            ...
            if (nounit) {
                const re = tempRe * a.r[coorAJ + j] - tempIm * a.i[coorAJ + j];
                const im = tempRe * a.i[coorAJ + j] + tempIm * a.r[coorAJ + j];
                tempRe = re;
                tempIm = im;
            }
            for (let i = 1; i <= m; i++) {
            ...
            for (let k = 1; k <= j - 1; k++) {
                ...
                if (!aIsZero) {
                    //TEMP = ALPHA * A(K, J)
                    const tempRe = alpha.re * a.r[coorAJ + k] - alpha.im * a.i[coorAJ + k];
                    const tempIm = alpha.re * a.i[coorAJ + k] + alpha.im * a.r[coorAJ + k];
